...
`;
    ...
    if (machinesData.length > 0) {
      ...
      for (const machine of machinesData) {
        sql += `INSERT INTO machines (id, code, name, line_id, sub_line_id, type, maintenance_frequency, pm_plan_year, uptime) VALUES (${formatValue(machine.id)}, ${formatValue(machine.code)}, ${formatValue(machine.name)}, ${formatValue(machine.lineId)}, ${formatValue(machine.subLineId)}, ${formatValue(machine.type)}, ${formatValue(machine.maintenanceFrequency)}, ${formatValue(machine.pmPlanYear)}, ${formatValue(machine.uptime)});\n`;
      }
    }
    ...
    if (breakdownsData.length > 0) {
      ...
      for (const breakdown of breakdownsData) {
        sql += `INSERT INTO breakdowns (id, date, shift, line_id, sub_line_id, machine_id, problem_type_id, priority, action_taken, root_cause, start_time, finish_time, total_minutes, major_contribution, major_contribution_time, attend_by_id, closed_by_id, remark, status, capa_required, capa_operator, capa_maintenance, capa_what_happened, capa_failure_mode, capa_sketch, capa_problem_descriptions, capa_root_causes, capa_preventive_actions, capa_prepared_by, capa_checked_by, capa_reviewed_by, created_by_id, created_at, updated_at) VALUES (${formatValue(breakdown.id)}, ${formatValue(breakdown.date)}, ${formatValue(breakdown.shift)}, ${formatValue(breakdown.lineId)}, ${formatValue(breakdown.subLineId)}, ${formatValue(breakdown.machineId)}, ${formatValue(breakdown.problemTypeId)}, ${formatValue(breakdown.priority)}, ${formatValue(breakdown.actionTaken)}, ${formatValue(breakdown.rootCause)}, ${formatValue(breakdown.startTime)}, ${formatValue(breakdown.finishTime)}, ${formatValue(breakdown.totalMinutes)}, ${formatValue(breakdown.majorContribution)}, ${formatValue(breakdown.majorContributionTime)}, ${formatValue(breakdown.attendById)}, ${formatValue(breakdown.closedById)}, ${formatValue(breakdown.remark)}, ${formatValue(breakdown.status)}, ${formatValue(breakdown.capaRequired)}, ${formatValue(breakdown.capaOperator)}, ${formatValue(breakdown.capaMaintenance)}, ${formatValue(breakdown.capaWhatHappened)}, ${formatValue(breakdown.capaFailureMode)}, ${formatValue(breakdown.capaSketch)}, ${formatValue(breakdown.capaProblemDescriptions)}, ${formatValue(breakdown.capaRootCauses)}, ${formatValue(breakdown.capaPreventiveActions)}, ${formatValue(breakdown.capaPreparedBy)}, ${formatValue(breakdown.capaCheckedBy)}, ${formatValue(breakdown.capaReviewedBy)}, ${formatValue(breakdown.createdById)}, ${formatValue(breakdown.createdAt)}, ${formatValue(breakdown.updatedAt)});\n`;
      }
    }
